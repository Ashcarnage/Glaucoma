...
class Encoder():
    ...
    def build(self):
        ...
        self.model = tf.keras.Model(inputs=inputs,outputs = p)
        self.model.summary()
        # bridge 
...
# Define the U-Net model with loops
def unet_model(input_shape, num_classes):
    ...
    # Decoder
    for filters, encoder_layer in zip([512, 256, 128, 64], reversed(encoder_layers)):
        x = UpSampling2D((2, 2))(x)
        try:
            x = Concatenate()([x, encoder_layer])
        except:
            pad_height = 1 - tf.shape(x)[1] % 2  # Calculate how much padding is needed to make it odd
            x = tf.pad(x, [[0, 0], [0, pad_height], [0, 0], [0,0]])
            x = Concatenate()([x, encoder_layer])
